...
# Hay que mirar el offset de la camara con la carga. Donde esta la documentacion de esta funcion??
...
def mission():
    # justo despues de la transicion a vtol
    carga = True
    while not lander(carga):
        print("ID no encontrado")
    # parar motores y soltar carga cuando llegue (no se cuando llega)
    carga = False
    #arm_and_takeoff()  a una altura prudencial
    while not lander(carga):
        print("ID no encontrado")
        # Sube un poco mas!!!
    # parar motores y coger carga cuando llegue (no se cuando llega)
    carga = True
